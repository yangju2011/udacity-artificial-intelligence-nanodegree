...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        # when adding a level, there is not a_level[level] yet, so need to first append set()                        
        self.a_levels.append(set())
        for action in self.all_actions:
            # create a new action node
            anode = PgNode_a(action)
            # verify that it meets the preconditions as a subset 
            prenodes = anode.prenodes
            subset = True
            for p in prenodes:
                if p not in self.s_levels[level]:
                    subset = False
            if subset == True: # A&B == A , A is a subset of B
                self.a_levels[level].add(anode)
                # establish connection between state and action levels
                for snode in self.s_levels[level]:
                    if snode in prenodes:
                        # add connections
    ...
    def add_literal_level(self, level):
        ...
        if level > 0:
            ...
            for anode in self.a_levels[level-1]:
                for effnode in anode.effnodes: # possible children node only
                    self.s_levels[level].add(effnode)
                    anode.children.add(effnode)
                    effnode.parents.add(anode)                                   
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        a1 = node_a1.action
        a2 = node_a2.action
        a1_pre = a1.precond_pos 
        a2_pre = a2.precond_pos
        # negate effect result
        a1_eff = a1.effect_rem
        a2_eff = a2.effect_rem
        # return True is negate
        for p in a1_pre: # make sure prec is not in rem, not the reverse
            if p in a2_eff:
                return True
        for p in a2_pre:
            if p in a1_eff:
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        a2_eff = a2.effect_rem
        # return True is negate
        for p in a2_eff: # reverse of inconsistent, test effect to precond
            if p in a1_pre:
                return True
        for p in a1_eff:
            if p in a2_pre:
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        node_s1 = node_a1.parents
        node_s2 = node_a2.parents
        # use parents, not precond because all parents have more fluents than just precond
        # if share the same parents, even when precond is irrelavant, still competing
        for s1 in node_s1:
            for s2 in node_s2:
                if s1.is_mutex(s2): return True       
        return False
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        # parent action sets, piarwise mutex
    ...
    def h_levelsum(self) -> int:
        ...
        # goal is problem.goal a list of fluents
        goals = self.problem.goal # state is TF format, goal contains only positive fluent, only contains positive fluent     
        goal_dict = {}
        for cost, s_set in enumerate(self.s_levels): # each level is a set of states node_s
            for node_s in s_set: 
                # symbol : str A string representing a literal expression from a planning problem ,  is_pos : bool
                if node_s.is_pos == True and node_s.symbol in goals and node_s.symbol not in goal_dict:
                    goal_dict[node_s.symbol] = cost
        for k in goal_dict: level_sum += goal_dict[k]
        if len(goal_dict) != len(goals): return float("inf") # not all goals are found
        return level_sum
